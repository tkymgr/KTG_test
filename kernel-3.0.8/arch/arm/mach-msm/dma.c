...
DEFINE_TIMER(timer, msm_dmov_clock_timer, 0, 0);
...
void msm_dmov_enqueue_cmd_ext(unsigned id, struct msm_dmov_cmd *cmd)
{
	...
	spin_lock_irqsave(&dmov_conf[adm].lock, irq_flags);
	if (dmov_conf[0].clk_ctl == CLK_DIS)
		msm_dmov_clk_toggle(0, 1);
	else if (dmov_conf[0].clk_ctl == CLK_TO_BE_DIS)
		del_timer(&dmov_conf[0].timer);
	dmov_conf[0].clk_ctl = CLK_EN;
	...
	if (status & DMOV_STATUS_CMD_PTR_RDY) {
		...
		if (!dmov_conf[adm].channel_active) {
			dmov_conf[0].clk_ctl = CLK_TO_BE_DIS;
			mod_timer(&dmov_conf[0].timer, jiffies + HZ);
		}
...
static irqreturn_t msm_datamover_irq_handler(int irq, void *dev_id)
{
	...
	if (!dmov_conf[adm].channel_active) {
		disable_irq_nosync(dmov_conf[0].irq);
		dmov_conf[0].clk_ctl = CLK_TO_BE_DIS;
		mod_timer(&dmov_conf[0].timer, jiffies + HZ);
	}
...
static int __init msm_dmov_init_clocks(void)
{
	...
	dmov_conf[adm].clk = clk_get(NULL, "adm_clk");
